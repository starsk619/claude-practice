...
### 2026-08-06 — 장 시작 전/휴장일엔 "최근 완료 세션"의 등락률을 보여주도록 변경
- 사용자가 8/6(목) 리포트에서 대부분 종목이 "전일대비 0%"로 나오는 걸 발견 — GitHub
  Actions가 매일 08:00 KST에 도는데 KRX 개장은 09:00라서, 실행 시점엔 `currentPrice`가
  사실상 어제 종가 그대로였음. 지난번 휴장일 수정(타임스탬프 기반 판정)은 이 상황을
  "직전 거래일 종가와 같으니 변동 0%"로 **기술적으로는 맞게** 판정했지만, 매일 아침
  의미 없는 0%만 보여주는 부작용이 있었음. "9시로 스케줄을 옮길지" 검토했으나, 개장 직후
  얇은 데이터만 잡히고 뉴스 수집도 늦어져서 기각 — 대신 데이터 로직만 수정.
- `isLikelyKrxLiveSession(date)` 신규: KST 기준 평일 09:00~15:30인지 판정(공휴일 캘린더는
  반영 안 함 - 주말과 동일하게 "휴장"으로 자연스럽게 처리되므로 불필요).
- `previousClose` 판정을 3갈래로 재구성:
  1. 오늘자 봉이 이미 있음(장 마감 후 재조회) — 기존과 동일.
  2. 오늘자 봉 없음 + 지금이 장중으로 판단됨(실시간 조회) — 기존과 동일, `currentPrice` vs
     어제 종가.
  3. **(신규)** 오늘자 봉 없음 + 장중 아님(장 시작 전/장 마감 후/휴장일 전부) —
     `currentPrice`를 쓰지 않고, 가장 최근에 끝난 세션(`bars` 마지막 항목) 자체를 그 전
     세션과 비교. 화면에 보여줄 "현재가"도 이때는 그 세션의 (반올림된) 종가로 맞춰서,
     실시간이 아닌 상황에서 `meta.regularMarketPrice`의 미세한 노이즈로 값이 어긋나는 것도
     함께 방지(전날 논의했던 현대건설 사례와 같은 부류의 문제).
  라벨도 이 갈래에서는 "전일대비"라는 실시간 뉘앙스 대신 항상 "M/D(요일) 마감 기준"으로
  명시(요일은 `Intl.DateTimeFormat('ko-KR', {weekday:'short'})`).
- 부수 효과(의도된 것): 주말 실행도 이제 "0%"가 아니라 금요일 세션 자체의 실제 등락률이
  나온다 — 8/2(일) 리포트에서 봤던 "0%, 7/31 종가 대비"보다 유용한 정보로 바뀜.
- 모의(mock) 데이터로 `Date`를 특정 KST 시각(장중/장 시작 전/주말)에 고정시켜 3갈래 전부
  검증. 기존 회귀 테스트(52주/split/전일대비 라벨 등)도 이 변경에 맞춰 "언제 실행되는지"를
  고정하도록 손보고 전부 재통과 확인.
