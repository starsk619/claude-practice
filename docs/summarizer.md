...
### 2026-07-28 — 카테고리 요약 스키마에서 "(총 N건)" 표기 지시 제거
- 뉴스 요약 각 카테고리 끝에 "(총 N건)"이 표시되는 게 불필요하다는 사용자 피드백. 이전에도
  한 번 제거했었는데(별도 커밋), 카테고리 8개 확장 시 `schema.js`(위 항목 참고)의 응답 스키마
  필드 description에 같은 지시("마지막 줄에 '(총 N건)' 표기")가 재도입되면서 다시 나타난
  회귀였음 — 스키마의 description 문구가 실제 출력에 직접 영향을 준다는 걸 보여준 사례.
- `schema.js`의 해당 필드 description에서 그 지시만 제거(다른 규칙은 그대로 유지).
