...
### 2026-07-29 — 판단 유형별 누적 성과(자기 보정) 프롬프트 반영
- `analyzeInvestment`/`buildUserPrompt`에 5번째 인자 `ratingPerformance` 추가
  (`src/pickHistory/trackRecord.js`의 `computeRatingPerformance` 결과 — 등급별 누적 적중률/
  평균 수익률, 표본 부족한 등급은 `null`). `userPrompt.js`에 "최근 판단 유형별 누적 성과"
  섹션을 신설.
- `systemPrompt.js`에 지침 추가: 표본이 확보된 등급은 적중률에 따라 판단 기준(등급 부여
  엄격도, confidence)을 스스로 보정하고, "데이터 부족" 등급은 무시하도록 명시. 단, 오늘의
  실제 뉴스/시세 근거(원칙 1)보다 과거 성과를 우선하지 말라는 우선순위도 함께 명시.
- **중요한 한계**: 이건 모델을 재훈련/파인튜닝하는 게 아니라 프롬프트 레벨의 자기 보정
  지시일 뿐이다 — Gemini는 매일 독립 호출되는 stateless 모델이라 실제 학습 루프는 없고,
  모델이 이 지시를 실제로 반영한다는 보장도 없다(일 변동성 인용 지시 사례와 동일한 한계).
  그래서 코드가 rating/confidence를 강제로 덮어쓰는 로직은 의도적으로 넣지 않았다.
- 모의(mock) 데이터로 등급별 표본 있음/데이터부족 두 케이스 모두 프롬프트에 올바른 문구가
  들어가는지 확인. 자세한 집계 로직은 `docs/pickHistory.md` 참고.

### 2026-08-01 — 등락률 기준일 라벨(previousCloseLabel) 반영
- `priceData`의 휴장일 등락률 오표시 수정(`docs/priceData.md` 참고)에 맞춰, `userPrompt.js`의
  `formatMarketContextEntry`/`formatFxContext`가 하드코딩된 "전일대비" 대신 데이터가 제공하는
  `previousCloseLabel`("전일대비" 또는 "N/D 종가 대비")을 그대로 쓰도록 변경.
- `systemPrompt.js`에 지침 추가: rationale에서 등락률을 인용할 때 데이터에 주어진 라벨을
  그대로 쓰고, 임의로 "전일대비"라고 바꿔 쓰거나 서로 다른 기준일의 수치(예: 오늘자 등락률과
  "지난 리포트 대비" 가격 비교)를 같은 문장에서 마치 같은 기준인 것처럼 섞어 쓰지 말라고 명시
  — 실제로 모델이 "지난 리포트(259,500원) 대비 +26.81%"처럼 서로 다른 두 비교를 뒤섞어
  모순된 문장을 쓴 사례가 실전 리포트에서 발견됨.

### 2026-08-06 — "주의" 등급 기준 강화 (밸류에이션 부담만으로는 부여 금지)
- 실전 트랙레코드에서 "주의" 적중률이 0%(15건 중 0건), 평균 수익률은 오히려 +17.71%로
  나온 걸 사용자가 발견 — 삼성전기·알테오젠처럼 "밸류에이션이 과도하게 부담스럽다"는
  이유로 "주의"를 준 종목들이, 극단적인 반도체 랠리에 휩쓸려 다 같이 급등해버린 것.
  "고평가/리스크 존재"와 "실제로 하락 예상"이 같은 라벨("주의")로 뒤섞여 쓰이고 있었고,
  트랙레코드의 "주의 = 하락 예상" 채점 기준과 실제 사용 방식이 어긋나 있었음.
- 4번째 등급("관심") 신설도 검토했으나 채택하지 않음 — 애매한 중간 등급은 결국 확실한
  판단을 피해가는 도피처가 되기 쉽고(트랙레코드 채점 대상에서 빠지므로 검증도 안 받게 됨),
  이미 "관망"이 그 역할(방향 확신 없음, 채점 제외)을 하고 있어 새 등급 없이도 근거 문장에
  뉘앙스를 담을 수 있다고 판단해 기존 3단계를 유지하기로 함.
- `systemPrompt.js`에 지침 추가: "주의"는 PER/PBR이 높다는 이유만으로 주지 말고, 실제
  하락 가능성을 볼 구체적 근거(부정적 뉴스/실적 둔화/부정적 촉매/모멘텀 약화 신호)가 있을
  때만 사용. 밸류에이션 부담은 있지만 모멘텀이 강하고 뚜렷한 하락 촉매가 없는 종목은
  "관망"으로 분류하되, 근거 문장에 모멘텀-밸류에이션 긴장관계를 명시적으로 서술하도록 지시.
  "자기 보정" 지침에도 "주의 적중률이 낮으면 이 기준을 다시 점검하라"는 연결 문장 추가.
- 등락률 기준일 라벨 설명(추가 지침 상단)도 갱신 — `priceData`가 새로 반환하는 "M/D(요일)
  마감 기준" 라벨(장 시작 전/휴장일용, 자세한 내용은 `docs/priceData.md` 참고)을 모델이
  올바르게 인식하고 인용하도록 세 가지 라벨 형태를 모두 설명.
- 스키마(`schema.js`)나 리포터(`ratings.js`/배지/용어설명)는 건드리지 않음 — 등급 값
  자체는 "매수 고려"/"관망"/"주의" 세 개 그대로 유지.
- 프롬프트 텍스트 변경이라 로컬 모의 테스트로는 행동 변화를 검증할 수 없음(일 변동성 인용
  지시를 추가했을 때와 동일) — 다음 실전 GitHub Actions 리포트에서 "주의" 종목이 실제
  하락 근거를 갖추고 있는지, 밸류에이션-부담만인 종목이 "관망"으로 재분류되는지 확인 필요.
